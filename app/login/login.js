...
import {
    View, Image, KeyboardAvoidingView, TouchableOpacity, Alert
} from 'react-native';
...
export default Login = () => {    
    ...
    const [userDetails, setUserDetails] = useState({ roomMail: "", roomName: "", status: "failure" })
    ...
    const redirectFunc = (val) => {
        if (val.status == 'success') {
            navigation.navigate('MainScreen', { roomMail: val.roomMail, roomName: val.roomName });
        } else {
    ...
    const submitButtonFun = () => {
        if ((password.length > 0 && userName.length > 0)) {
            //login();
            navigation.navigate('MainScreen',{roomMail: "basfadmin@gmail.com", roomName: "Guoco Midtown" });
        } else {
